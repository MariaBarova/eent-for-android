...
public class Kingdom
{
	...
	public void removeGridFromKingdom(Grid grid)
	{
		...
		Vector temp_vector = wanderKingdoms(null);
		switch (tile.getTileType())
		{
			...
			switch (((LeadTile)tile).getLeaderType())
			{
				...
				if (king == HAS_LEADER)
				{
					leaders[LeadTile.LEADER_KING - LeadTile.LEAD].removeBoardTile();
				...
				else if (king == OTHER_KINGDOM)
				{
					for(int i = 0; i < temp_vector.size(); i++)
					{
						...
						{
							((Kingdom)temp_vector.elementAt(i)).removeGridFromKingdom(grid);
						}
					}
				}
				break;
				...
				{
					leaders[LeadTile.LEADER_PRIEST - LeadTile.LEAD].removeBoardTile();
				...
				else if (priest == OTHER_KINGDOM)
				{
					for(int i = 0; i < temp_vector.size(); i++)
					{
						...
						{
							((Kingdom)joinedWith.elementAt(i)).removeGridFromKingdom(grid);
						}
					}
				}
				break;
				...
				{
					leaders[LeadTile.LEADER_TRADER - LeadTile.LEAD].removeBoardTile();
				...
				{
					leaders[LeadTile.LEADER_FARMER - LeadTile.LEAD].removeBoardTile();
	...
	public void addLeader(Grid grid)
	{
		...
		Kingdom previous_kingdom = grid.getKingdom();
		/* debug info
		System.out.println("trying to add leader from " + grid.getActionCommand() +
				   " to " + this);
		System.out.println(grid.getActionCommand() + " in " + grid.getKingdom()+
				   " is " + grid.inKingdom());
		...
		*/
		switch(lead.getLeaderType())
		{
			...
			if (hasKing() == NO_LEADER)
			{
				grid.setKingdom(this);
				/* debug info
				System.out.println("set kingdom of " + grid.getActionCommand() + " to " + this);
				*/
	...
	/** return the Leader_king in the kingdom if it is
	* not found in the current kingdom all joined kingdoms
	* are searched for the Leader_king
	*/
	public Grid getKing()
	{
		int king = hasKing();
	...
	/** return the Leader_priest in the kingdom if it is
	* not found in the current kingdom all joined kingdoms
	* are searched for the Leader_priest
	*/
	...
	/** return the Leader_trader in the kingdom if it is
	* not found in the current kingdom all joined kingdoms
	* are searched for the Leader_trader
	*/
	...
	/** return the Leader_farmer in the kingdom if it is
	* not found in the current kingdom all joined kingdoms
	* are searched for the Leader_farmer
	*/
	...
	/** return the total number of settlements in all
	* the joined kingdoms
	*/
	public int getTotalNumberOfSettlements()
	{
		...
		for (int i = 0; i < temp_vector.size(); i++)
		{
			temp = temp + ((Kingdom)temp_vector.elementAt(i)).getNumberOfSettlements();
		}
		return temp;
	}
	...
	/** return the total number of farms in all
	* the joined kingdoms
	*/
	public int getTotalNumberOfFarmers()
	{
		...
		for (int i = 0; i < temp_vector.size(); i++)
		{
			temp = temp + ((Kingdom)temp_vector.elementAt(i)).getNumberOfFarmers();
		}
		return temp;
	}
	...
	/** return the total number of temples in all
	* the joined kingdoms
	*/
	public int getTotalNumberOfTemples()
	{
		...
		for (int i = 0; i < temp_vector.size(); i++)
		{
			temp = temp + ((Kingdom)temp_vector.elementAt(i)).getNumberOfTemples();
		}
		return temp;
	}
	...
	/** return the total number of markets in all
	* the joined kingdoms
	*/
	public int getTotalNumberOfMarkets()
	{
		...
		for (int i = 0; i < temp_vector.size(); i++)
		{
			temp = temp + ((Kingdom)temp_vector.elementAt(i)).getNumberOfMarkets();
		}
		return temp;
	}
	
	/** join two vector with the result of a new vector that
	* has only the elements of each that are different
	*/
	private Vector unifyVectors(Vector vector_a, Vector vector_b)
	{
	...
	/** a vector with every kingdom this kingdom is joined with
	* that is directly and indirectly
	*/
	public Vector wanderKingdoms(Kingdom exclude_me)
	{
		Vector joinedKingdoms = new Vector();
		/*
		if (times > 10)
		{
		...
		else
		{
			...
			*/
			...
			{
				if (exclude_me != null && joinedWith.elementAt(i).equals(exclude_me))
				{
					
				}
				else
				{
					joinedKingdoms = unifyVectors(joinedKingdoms,((Kingdom)joinedWith.elementAt(i)).wanderKingdoms(this));
				}
			}
			return joinedKingdoms; 
		//}
	...
	public Vector getJoinedKingdoms()
	{
		return joinedWith;
	}
	...
	*/
	...
	/** check to see if this kingdom is joined with
	* a kingdom
	*/
