...
public class Board extends JDesktopPane 
{
	...
	private static final int unificationcount=1;
	...
	public Board()
	{
		...
		images[0]  = new ImageIcon(getClass().getResource("images/Settlement.gif"));
		...
		images[9]  = new ImageIcon(getClass().getResource("images/playerking.gif"));
		images[10] = new ImageIcon(getClass().getResource("images/playerpriest.gif"));
		images[11] = new ImageIcon(getClass().getResource("images/playerfarmer.gif"));
		images[12] = new ImageIcon(getClass().getResource("images/playertrader.gif"));
		images[13] = new ImageIcon(getClass().getResource("images/computerking.gif"));
		images[14] = new ImageIcon(getClass().getResource("images/computerpriest.gif"));
		images[15] = new ImageIcon(getClass().getResource("images/computerfarmer.gif"));
		images[16] = new ImageIcon(getClass().getResource("images/computertrader.gif"));
		images[17] = new ImageIcon(getClass().getResource("images/monument1.gif"));
		images[18] = new ImageIcon(getClass().getResource("images/monument2.gif"));
		images[19] = new ImageIcon(getClass().getResource("images/monument3.gif"));
		images[20] = new ImageIcon(getClass().getResource("images/monument4.gif"));
		images[21] = new ImageIcon(getClass().getResource("images/unif.gif"));
		
		paintGrids();
		/* debug info
		System.out.println(kingdomcount+" kingdoms have been created");
		*/
	}
	...
	public static boolean isNearATemple(Grid sel)
	{
		...
		int col = Integer.parseInt(cmd.substring(cmd.indexOf('-')+1, cmd.length()-1));
		around = new  Grid [8];
		for (int i=0; i < 8; i++)
		{
			around[i]=null;
		...
		//System.out.println(around[above].getGridType()+">"+around[under].getGridType()+">"+around[left].getGridType()+">"+around[right].getGridType());
		//System.out.println(around[under_right].getActionCommand()+">"+around[above_right].getActionCommand()+">"+around[under_left].getActionCommand()+">"+around[above_left].getActionCommand());
	...
	public static void Monument(Grid selc)
	{
		if (monument > 0)
		{
			...
			if (check1 && check3)
			{
				if (selc.getGridType() == (Board.grids[row][col+1]).getGridType()
				&& selc.getGridType() == (Board.grids[row+1][col]).getGridType()
				&& selc.getGridType()==(Board.grids[row+1][col+1]).getGridType())
				{
					monument--;
			...
			if (check1 && check4)
			{
				if (selc.getGridType()==(Board.grids[row-1][col]).getGridType()
				&& selc.getGridType()==(Board.grids[row][col+1]).getGridType()
				&& selc.getGridType()==(Board.grids[row-1][col+1]).getGridType()) 
				{
					monument--;
	...
	public static Vector checkForKingdoms(Grid grid)
	{
	...
	/** get de status of a grid to see if a problem will arise
	* meaning a conflict between player or a unification a two
	* main temples
	*/
	public static int checkKingdom(Grid from_grid, Grid to_grid)
	{   
		//System.out.println("from "+ from_grid.getActionCommand());
                //System.out.println("to "+ to_grid.getActionCommand());
		Tile from_tile = from_grid.getTile();
		Vector temp_vector = checkForKingdoms(to_grid);
			...
			else if (temp_vector.size() == 1)
			{
				...
				if (first_kingdom.hasLeader())
				{
					switch(((LeadTile)from_tile).getLeaderType())
					{
						...
						if (first_kingdom.hasKing() >= Kingdom.HAS_LEADER
						&& !(((first_kingdom.getKing()).getTile()).equals(from_tile)))
						{
						...
						&& !(((first_kingdom.getPriest()).getTile()).equals(from_tile)))
						{
							return INTERNAL_CONFLICT;
						}
						else
						...
						&& !(((first_kingdom.getTrader()).getTile()).equals(from_tile)))
						{
							return INTERNAL_CONFLICT;
						}
						else
						...
						&& !(((first_kingdom.getFarmer()).getTile()).equals(from_tile)))
						{
							return INTERNAL_CONFLICT;
						}
						else
	...
	/** gives points to a player based on the leader in
	* the kingdom
	*/
		...
		if (g.inKingdom())
		{
			...
			if (kingdom.hasLeader())
			{
				switch (((CivTile)g.getTile()).getCivType())
				{
					...
					{
						owner = ((LeadTile)((kingdom.getPriest()).getTile())).getPlayer();
						owner.incr_score(Player.TEMPLE_POINT);
						if (owner.isCPU())
						{
							System.out.println(owner.getTemplePoints()+" computer player red score");
						}
						else
						{
					...
					{
						owner = ((LeadTile)((kingdom.getKing()).getTile())).getPlayer();
						owner.incr_score(Player.TEMPLE_POINT);
						if (owner.isCPU())
						{
							System.out.println(owner.getTemplePoints()+" computer player black score");
						}
						else
						{
							Tigris2.ScoreBoard2.setText(""+ owner.getTemplePoints());
					...
					{
						owner = ((LeadTile)((kingdom.getFarmer()).getTile())).getPlayer();
						owner.incr_score(Player.FARM_POINT);
						if (owner.isCPU())
						{
							System.out.println(owner.getFarmPoints()+" computer player blue score");
						}
						else
						{
					...
					else if (kingdom.hasKing() >= Kingdom.HAS_LEADER)
					{
						...
						if (owner.isCPU())
						{
							System.out.println(owner.getFarmPoints()+" computer player green score");
						}
						else
						{
							Tigris2.ScoreBoard3.setText(""+owner.getFarmPoints());
					...
					if (kingdom.hasKing() >= Kingdom.HAS_LEADER)
					{
						...
						if (owner.isCPU())
						{
							System.out.println(owner.getPeoplePoints()+" computer player black score");
						}
						else
						{
					...
					{
						owner = ((LeadTile)((kingdom.getTrader()).getTile())).getPlayer();
						owner.incr_score(Player.MARKET_POINT);
						if (owner.isCPU())
						{
							System.out.println(owner.getMarketPoints()+" computer player blue score");
						}
						else
						{
	...
	{
		Vector temp_vector = checkForKingdoms(to_grid);
		int gridNumber = Integer.parseInt((from_grid.getActionCommand()).substring(0,1));
		Kingdom first_kingdom = (Kingdom)temp_vector.elementAt(0);
		Kingdom second_kingdom = (Kingdom)temp_vector.elementAt(1);
	...
	{
		/*
		*attack is cuurent player
		*selec is the selected grid that started the conflict
		*/
		...
		/* debug info
		System.out.println("att "+attackleader);
		System.out.println("def "+defendleader);
		System.out.println("def kingd "+temp_king);
		...
		*/
		...
		{
			//  System.out.println("around attack"+temp_attack[i].getGridType());
			if (temp_attack[i] != null) 
			{ 
				if (temp_attack[i].getGridType()==Grid.TEM || temp_attack[i].getGridType()==Grid.TERS)
				{
					attackCount++;
				}
			}
		...
		{
			// System.out.println("around def"+temp_defen[i].getGridType());
			if (temp_defen[i] != null)  
			{ 
				if (temp_defen[i].getGridType()==Grid.TEM || temp_defen[i].getGridType()==Grid.TERS)
				{
					defendCount++;
				}
			}
		}
		//System.out.println("attack kingdom  "+selec.getKingdom());
		//System.out.println("def kingdom  "+defendleader.getKingdom());
		...
		if (attackCount > defendCount)
		{
			...
			if (attacker.isCPU())
			{
				System.out.println(attacker.getTemplePoints()+" computer player red score");
				//Tigris2.infolabel.setText("Computer player wins the conflict ");
				JOptionPane.showMessageDialog(null,"Computer player wins the conflict ");
			}
			else
			{
				Tigris2.ScoreBoard2.setText(""+attacker.getTemplePoints());
				//Tigris2.infolabel.setText("Human player wins the conflict ");
				JOptionPane.showMessageDialog(null,"Human player wins the conflict");
			}
		}
		...
		else if ( defendCount >= attackCount)
		{
			...
			if (defender.isCPU())
			{
				System.out.println(defender.getTemplePoints()+" computer player red score");
				//Tigris2.infolabel.setText("Computer player wins the conflict ");
				JOptionPane.showMessageDialog(null,"Computer player wins the conflict ");
			}
			else
			...
			//else if (defendCount == attackCount) {System.out.println(defender+"wins");
		}
	}
	
	/** search voor the grid with the gridtype by the findtile int 
	* returns the grid if found 
	*else null is returned
	*/  
	public static Grid scan(int findtile)
	{
		int i =0;
		Grid [] near = new Grid [4];
		while (i < 11) 
		{
			int j = 0;
			while (j < 16) 
	...
	public static boolean externalConflict(Player attack,Grid selec)
	{
		...
		for (int i = 0; i < temp.length; i++)
		{
			...
			else if (temp[i].inKingdom())
			{
				if (temp_vector.contains(temp[i].getKingdom()))
				{
					//do nothing
				}
				else
		...
		if (first_kingdom !=null && second_kingdom!=null)
		{
			...
			{
				//get the settelemts that each kingdom has and check if the first kingdom hase more settelemnt tiles   
				if (first_kingdom.getNumberOfSettlements() > second_kingdom.getNumberOfSettlements())
				{
					...
					if (((LeadTile)king1.getTile()).getPlayer()== attacker)
					{
						...
						for (int i=0; i <= second_kingdom.getNumberOfSettlements();i++)
						{
						...
						{  
							...
							{
								temp_lead_tile[i] = new LeadTile(((LeadTile)king2.getTile()).getLeaderType(), defender);
								second_kingdom.removeGridFromKingdom(king2); 
								//update the loser`s lead tiles
								defender.setLeaders(temp_lead_tile);
								
								// if the losser is human player repaint the leaders
								if (!defender.isCPU())
								{
									LeadTile[] templead = defender.getLeaders();
									for (int ii =0; ii < 4; i++)
					...
					else if (((LeadTile)king1.getTile()).getPlayer()== defender)
					{
						...
						{  
							if (temp_lead_tile[i] == null)
							{
								temp_lead_tile[i] = new LeadTile(((LeadTile)king2.getTile()).getLeaderType(), attacker);
								second_kingdom.removeGridFromKingdom(king2); 
				...
				//get the settelemts that each kingdom has and check if the second kingdom hase more settelemnt tiles   
				else if (first_kingdom.getNumberOfSettlements() < second_kingdom.getNumberOfSettlements())
				{
					...
					if (((LeadTile)king2.getTile()).getPlayer()== attacker)
					{
						...
						{  
							if (temp_lead_tile[i] == null)
							{
								temp_lead_tile[i] = new LeadTile(((LeadTile)king1.getTile()).getLeaderType(), defender);
					...
					else if (((LeadTile)king2.getTile()).getPlayer()== defender)
					{
						...
						{  
							if (temp_lead_tile[i] == null)
							{
								temp_lead_tile[i] = new LeadTile(((LeadTile)king1.getTile()).getLeaderType(), attacker);
								first_kingdom.removeGridFromKingdom(king1); 
								attacker.setLeaders(temp_lead_tile);
			...
			{
				if (first_kingdom.getNumberOfTemples() > second_kingdom.getNumberOfTemples())
				{
					if (((LeadTile)priest1.getTile()).getPlayer()== attacker)
					{
						...
						{  
							if (temp_lead_tile[i] == null)
							{
								temp_lead_tile[i] = new LeadTile(((LeadTile)priest2.getTile()).getLeaderType(), defender);
					...
					else if (((LeadTile)priest1.getTile()).getPlayer()== defender)
					{
						...
						{  
							if (temp_lead_tile[i] == null)
							{
								temp_lead_tile[i] = new LeadTile(((LeadTile)priest2.getTile()).getLeaderType(), attacker);
								second_kingdom.removeGridFromKingdom(priest2); 
								attacker.setLeaders(temp_lead_tile);
				...
				else if (first_kingdom.getNumberOfTemples() < second_kingdom.getNumberOfTemples())
				{
					if (((LeadTile)priest2.getTile()).getPlayer()== attacker)
					{
						...
						{  
							if (temp_lead_tile[i] == null)
							{
								temp_lead_tile[i] = new LeadTile(((LeadTile)priest1.getTile()).getLeaderType(), defender);
					...
					else if (((LeadTile)priest2.getTile()).getPlayer()== defender)
					{
						...
						{  
							if (temp_lead_tile[i] == null)
							{
								temp_lead_tile[i] = new LeadTile(((LeadTile)priest1.getTile()).getLeaderType(), attacker);
								first_kingdom.removeGridFromKingdom(priest1); 
								attacker.setLeaders(temp_lead_tile);
			...
			{
				if (first_kingdom.getNumberOfFarmers() > second_kingdom.getNumberOfFarmers())
				{
					if (((LeadTile)farmer1.getTile()).getPlayer()== attacker)
					{
						...
						{  
							if (temp_lead_tile[i] == null)
							{
								temp_lead_tile[i] = new LeadTile(((LeadTile)farmer2.getTile()).getLeaderType(), defender);
					...
					else if (((LeadTile)farmer1.getTile()).getPlayer()== defender)
					{
						...
						{  
							if (temp_lead_tile[i] == null)
							{
								temp_lead_tile[i] = new LeadTile(((LeadTile)farmer2.getTile()).getLeaderType(), attacker);
								second_kingdom.removeGridFromKingdom(farmer2); 
								attacker.setLeaders(temp_lead_tile);
				...
				else if (first_kingdom.getNumberOfFarmers() < second_kingdom.getNumberOfFarmers())
				{
					if (((LeadTile)farmer2.getTile()).getPlayer()== attacker)
					{
						...
						{  
							if (temp_lead_tile[i] == null)
							{
								temp_lead_tile[i] = new LeadTile(((LeadTile)farmer1.getTile()).getLeaderType(), defender);
					...
					else if (((LeadTile)farmer2.getTile()).getPlayer()== defender)
					{
						...
						{  
							if (temp_lead_tile[i] == null)
							{
								temp_lead_tile[i] = new LeadTile(((LeadTile)farmer1.getTile()).getLeaderType(), attacker);
								first_kingdom.removeGridFromKingdom(farmer1); 
								attacker.setLeaders(temp_lead_tile);
			...
			{
				if (first_kingdom.getNumberOfMarkets() > second_kingdom.getNumberOfMarkets())
				{
					if (((LeadTile)trader1.getTile()).getPlayer()== attacker)
					{
						...
						{  
							if (temp_lead_tile[i] == null)
							{
								temp_lead_tile[i] = new LeadTile(((LeadTile)trader2.getTile()).getLeaderType(), defender);
					...
					else if (((LeadTile)trader1.getTile()).getPlayer()== defender)
					{
						...
						{  
							if (temp_lead_tile[i] == null)
							{
								temp_lead_tile[i] = new LeadTile(((LeadTile)trader2.getTile()).getLeaderType(), attacker);
								second_kingdom.removeGridFromKingdom(trader2); 
								attacker.setLeaders(temp_lead_tile);
				...
				else if (first_kingdom.getNumberOfMarkets() < second_kingdom.getNumberOfMarkets())
				{
					if (((LeadTile)trader2.getTile()).getPlayer()== attacker)
					{
						...
						{  
							if (temp_lead_tile[i] == null)
							{
								temp_lead_tile[i] = new LeadTile(((LeadTile)trader1.getTile()).getLeaderType(), defender);
					...
					else if (((LeadTile)trader2.getTile()).getPlayer()== defender)
					{
						...
						{  
							if (temp_lead_tile[i] == null)
							{
								temp_lead_tile[i] = new LeadTile(((LeadTile)trader1.getTile()).getLeaderType(), attacker);
								first_kingdom.removeGridFromKingdom(trader1); 
								attacker.setLeaders(temp_lead_tile);
	...
	public static int getSupport(int type,Player player)
	{
		...
		if (player.isCPU())
		{
			...
			//System.out.println("computer player supp"+compp_supp);
		}
		else
		{
